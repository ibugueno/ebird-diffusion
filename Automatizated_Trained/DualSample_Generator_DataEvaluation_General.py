...
def Metrics(args,number):
    ...
    ########################### METRICS #############################################
    groth_truth_folder = os.path.join(conditional_config['path_ground_truth'], str(number))
    condition_folder = os.path.join(conditional_config['path_condition'], str(number))
    
    print("Test Image Folder: ",groth_truth_folder)
    print("Test Event Folder: ",condition_folder)
    ...
    for file in Common_files:  #Sacar el iter para generar en todo el conjunto y borrar el break de abajo
        iteration+= 1
        ...
        with torch.no_grad():
            ...
            save_rute = os.path.join(train_config['task_name'],"Generations_"+str(number))
            #print(save_rute)
            if not os.path.exists(save_rute):
                os.mkdir(save_rute)
            save_rute = os.path.join(save_rute, file)            
...
def run_metrics(args,route_yamls, number):
    # Parseamos los argumentos y ejecutamos la métrica
    Metrics(args, number)

def main(yamls, args):
    with ProcessPoolExecutor() as executor:
        # Creamos una lista de tareas a partir de las combinaciones de yamls y números
        futures = [
            executor.submit(run_metrics, args ,route_yamls, number)
            for route_yamls in yamls
            for number in range(0, 10)
        ]
        
        # Opcional: Verificar que todas las tareas se completen
        for future in futures:
            future.result()  # Esto captura excepciones si las hay
...
if __name__ == '__main__':
    ...
    yamls = ['Training_yaml/General.yaml']
    args = parser.parse_args(['--config', yamls[0]])
    main(yamls,args)
