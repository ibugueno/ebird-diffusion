...
def infer(args, subset_ratio_ = 0.05):
    ...
    print(conditional_config['path_conditional'])
    for route in conditional_config['path_conditional']:
        archivos = os.listdir(route)
        for archivo in archivos:
            full_route = route+"/"+archivo
            print(full_route)
            c = load_single_image(full_route).to(device)
            with torch.no_grad():
               class_name = route[-1]
               img = sample(Model_original ,model,c ,scheduler, train_config, model_config, diffusion_config, subset_ratio_, archivo, class_name )
...
if __name__ == '__main__':
    ...
    infer(args, subset_ratio_ = 0.25)
    infer(args, subset_ratio_ = 0.50)
    infer(args, subset_ratio_ = 0.75)
    infer(args, subset_ratio_ = 1.0)