...
def sample(Model_original, model, c ,scheduler, train_config, model_config, diffusion_config, subset_ratio_):
    r"""
    ...
    for i in tqdm(reversed(range(diffusion_config['num_timesteps']))):
        ...
        auxxx = 'samples_'+str(int(subset_ratio_*100))
        # Save x0
        ims = torch.clamp(xt, -1., 1.).detach().cpu()
        ims = (ims + 1) / 2
        grid = make_grid(ims, nrow=train_config['num_grid_rows'])
        img = torchvision.transforms.ToPILImage()(grid)
        if not os.path.exists(os.path.join(train_config['task_name'], auxxx)):
            os.mkdir(os.path.join(train_config['task_name'], auxxx))
        img.save(os.path.join(train_config['task_name'], auxxx, 'x0_{}.png'.format(i)))
        img.close()
    average_inference_time = sum_time / diffusion_config['num_timesteps']
    print(f"Tiempo de inferencia: {average_inference_time:.2f} ms")

def infer(args, subset_ratio_ = 0.05):
    ...
    combinet = "Rislab_Event_influence_volume/Combinet" + str(int(subset_ratio_*100)) + ".pth"
    ...
    with torch.no_grad():
        sample(Model_original ,model,c ,scheduler, train_config, model_config, diffusion_config, subset_ratio_)
...
if __name__ == '__main__':
    ...
    infer(args, subset_ratio_ = 0.05)
    infer(args, subset_ratio_ = 0.07)
